...
THREAD_POOL_TASK_FUNC(lnk_parse_debug_s_task)
{
  ...
  // FILECHKSMS merged above, so they stay consistent within this module.
  {
...
THREAD_POOL_TASK_FUNC(lnk_parse_debug_h_task)
{
  ...
  if (obj->debug_h_sect_idx < obj->header.section_count_no_null) {
    ...
    String8    raw_debug_h     = lnk_obj_get_sect_data(obj, obj->debug_h_sect_idx, section.frange);
  ...
  ProfScope("Prefetch CodeView")
  {
    ...
    for EachIndex(obj_idx, obj_count) {
      ...
      if (obj->debug_t_sect_idx < obj->header.section_count_no_null) {
        LNK_ObjSection sect = lnk_obj_section_from_sect_idx(obj, obj->debug_t_sect_idx);
        String8        data = lnk_obj_get_sect_data(obj, obj->debug_t_sect_idx, sect.frange);
      ...
      if (obj->debug_p_sect_idx < obj->header.section_count_no_null) {
        LNK_ObjSection sect = lnk_obj_section_from_sect_idx(obj, obj->debug_p_sect_idx);
        String8        data = lnk_obj_get_sect_data(obj, obj->debug_p_sect_idx, sect.frange);
        if (data.size) { ranges[range_count++] = rng_1u64((U64)data.str, (U64)data.str + data.size); }
      }
      if (config->ghash && obj->debug_h_sect_idx < obj->header.section_count_no_null) {
        LNK_ObjSection sect = lnk_obj_section_from_sect_idx(obj, obj->debug_h_sect_idx);
        String8        data = lnk_obj_get_sect_data(obj, obj->debug_h_sect_idx, sect.frange);
        if (data.size) { ranges[range_count++] = rng_1u64((U64)data.str, (U64)data.str + data.size); }
      }
    }
  ...
  CV_DebugT *debug_p_arr;
  {
    ...
    for EachIndex(obj_idx, obj_count) {
      ...
      if (obj->debug_t_sect_idx < obj->header.section_count_no_null) {
        ...
        raw_debug_t_arr[obj_idx].v[0]  = lnk_obj_get_sect_data(obj, obj->debug_t_sect_idx, debug_t_sect.frange);
      }
...
THREAD_POOL_TASK_FUNC(lnk_hash_debug_t_task)
{
  ...
  for EachIndex(leaf_idx, debug_t->count) {
    ...
    CV_Leaf      leaf    = cv_debug_t_get_leaf(debug_t, leaf_idx);
    CV_TiOffsets ti_offs = cv_leaf_ti_offsets(temp.arena, leaf.kind, leaf.data);
    lnk_hash_cv_leaf(task->input, (LNK_LeafRef){ obj_idx, leaf_idx }, ti_offs, 1);
    temp_end(temp);
  }
  ProfEnd();
}
...
THREAD_POOL_TASK_FUNC(lnk_hash_debug_t_deep_task)
{
  ...
  for EachIndex(leaf_idx, debug_t->count) {
    ...
    CV_Leaf      leaf    = cv_debug_t_get_leaf(debug_t, leaf_idx);
    CV_TiOffsets ti_offs = cv_leaf_ti_offsets(temp.arena, leaf.kind, leaf.data);
    lnk_hash_cv_leaf_deep(temp.arena, task->input, (LNK_LeafRef){ obj_idx, leaf_idx }, ti_offs);
    temp_end(temp);
  }
  ProfEnd();
}
...
THREAD_POOL_TASK_FUNC(lnk_count_unique_leaf_sizes_task)
{
  ...
  for EachInRange(i, range) {
    ...
    size += cv_debug_t_get_raw_leaf(debug_t, leaf_ref.leaf_idx).size;
  }
...
// lnk_cv_patcher_leaves_task (which patched TIs in-place into the mapped input, dirtying
// copy-on-write pages) with the old lnk_unbucket_raw_leaves_task (which pointed result.v into the
// input). result.v now points into the copy: identical bytes, identical order, clean input pages.
internal
...
lnk_merge_types(TP_Context *tp, TP_Arena *tp_temp, LNK_CodeViewInput *input, LNK_MergeTypeFlags merge_flags)
{
  ...
  ProfBegin("Fixup Type Indices");
  {
    ...
    // NOTE: leaf TI-fixup is fused into the unbucket/materialize pass below -- it copies each
  ...
  for EachIndex(ti_source, CV_TypeIndexSource_COUNT) {
    ...
    // materialize unique leaves into one contiguous private buffer, fixing up their type indices
    // during the copy (input leaves stay pre-fixup; dedup/hash phases already consumed them)
    task.leaf_buffer_offsets = push_array_no_zero(scratch.arena, U64, tp->worker_count);
    tp_for_parallel_prof(tp, 0, tp->worker_count, lnk_count_unique_leaf_sizes_task, &task, "Count Leaf Sizes");
    U64 leaf_buffer_size = 0;
    for EachIndex(i, tp->worker_count) {
      U64 range_size = task.leaf_buffer_offsets[i];
      task.leaf_buffer_offsets[i] = leaf_buffer_size;
      leaf_buffer_size += range_size;
    }
    task.leaf_buffer = push_array_no_zero(tp_temp->v[0], U8, leaf_buffer_size);
    tp_for_parallel_prof(tp, 0, tp->worker_count, lnk_materialize_unique_leaves_task, &task, "Materialize Leaves");
    ...
    for (U64 lane = task_id; lane < fixed_lane_count; lane += tp->worker_count) {
      ...
      for EachNode(chunk, LNK_SymbolHashTrieChunk, symbol_chunks.first) {
        for EachIndex(i, chunk->count) {
          ...
          // publish under the trie NODE's name, not the resolved symbol's: an /OPT:ICF-folded
          // external's node points at the fold leader's symbol while node->name keeps the folded
          // name -- emitting symbol->name printed the leader's name once per follower (~1.4M
          // byte-identical duplicate publics / 213 MB on a UE editor DLL), and the folded names
          // were absent entirely (name breakpoints on them could not bind). With the node name,
          // each follower emits an alias public at the leader's address, like link.exe.
          String8 pub_name = chunk->v[i].name ? *chunk->v[i].name : symbol->name;

          // drop residual byte-identical duplicates (chunk-order adjacent)
          ISectOff sc = lnk_sc_from_symbol(symbol);
          if (sc.isect == prev_sc.isect && sc.off == prev_sc.off && str8_match(prev_name, pub_name, 0)) { continue; }
          prev_name = pub_name;
          prev_sc   = sc;

          public_symbol_size  += AlignPow2(sizeof(CV_SymPub32) + pub_name.size + 1, sizeof(void *));
    ...
    for (U64 lane = task_id; lane < fixed_lane_count; lane += tp->worker_count) {
      ...
      for EachNode(chunk, LNK_SymbolHashTrieChunk, symbol_chunks.first) {
        for EachIndex(i, chunk->count) {
          ...
          // node name + dedup (must mirror the count pass above)
          String8  pub_name = chunk->v[i].name ? *chunk->v[i].name : symbol->name;
          ISectOff sc       = lnk_sc_from_symbol(symbol);
          if (sc.isect == prev_sc.isect && sc.off == prev_sc.off && str8_match(prev_name, pub_name, 0)) { continue; }
          prev_name = pub_name;
          ...
          CV_Symbol     pub_symbol = cv_make_pub32(public_symbol_arena, flags, safe_cast_u32(sc.off), safe_cast_u16(sc.isect), pub_name);
