  ...
  if (task_id == 0) {
    ...
    objs_by_idx = push_array_no_zero(scratch.arena, LNK_Obj *, objs_count ? objs_count : 1);
    {
      for EachIndex(obj_idx, objs_count) {
        ...
        is_live[obj_idx]     = push_array(scratch.arena, U8, obj->header.section_count_no_null + 1);
...
THREAD_POOL_TASK_FUNC(lnk_opt_icf_task)
{
  ...
  for EachInRange(contrib_idx, contrib_ranges[task_id]) {
    Contrib       *contrib    = &contribs[contrib_idx];
    ColorHashSlot *color_slot = &color_table.slots[contrib->color_slot_idx];
    Contrib       *leader     = &contribs[ins_atomic_u32_eval(&color_slot->first_contrib_idx)];
    ...
    if (contrib_obj->icf_fold) {
      contrib_obj->icf_fold[contrib->sect_idx] = (LNK_ICFFold){ .leader_obj_idx = (U32)leader->obj_idx, .leader_sn = (U32)(leader->sect_idx + 1), .set = 1 };
    }
...
lnk_opt_icf(TP_Context *tp, Arena *perm, LNK_SymbolTable *symtab, LNK_Config *config, LNK_Obj **objs, U64 objs_count)
{
  ...
  if (config->opt_ref == LNK_SwitchState_Yes) {
    ProfScope("Alloc fold maps") {
      for EachIndex(obj_idx, objs_count) {
        objs[obj_idx]->icf_fold = push_array(perm, LNK_ICFFold, objs[obj_idx]->header.section_count_no_null);
      }
    }
  }
...
THREAD_POOL_TASK_FUNC(lnk_icf_mark_folded_lines_task)
{
  ...
  for EachIndex(sect_idx, obj->header.section_count_no_null) {
    ...
    for EachIndex(assoc_idx, associated_sections.count) {
      ...
      obj->section_flags[assoc_sn - 1] |= COFF_SectionFlag_LnkRemove;
...
THREAD_POOL_TASK_FUNC(lnk_gather_sections_task)
{
  ...
  for EachInRange(obj_idx, range) {
    ...
    for EachIndex(sect_idx, obj->header.section_count_no_null) {
      ...
      // was section defined?
      COFF_SectionFlags      image_sect_flags     = obj->section_flags[sect_idx] & ~(COFF_SectionFlags_LnkFlags | COFF_SectionFlags_Reserved);
...
THREAD_POOL_TASK_FUNC(lnk_obj_reloc_patcher)
{
  ...
  for EachIndex(sect_idx, obj_header.section_count_no_null) {
    ...
    if (section_flags & LNK_SECTION_FLAG_DEBUG) {
      ...
      // Streaming-ring P3.3: .debug$S bytes stream through a small per-worker window at
      // module-write time; lnk_obj_window_debug_s re-reads the raw mapped section and applies
      // these same relocs (lnk_obj_apply_relocs_to_buffer, identical routine) into the window
      // there -- so the image-time patched copy (GB-class at FN scale) never exists. Other
      // reloc-bearing debug sections (.debug$T with relocs is essentially nonexistent, but
      // keep them exact) still take the copy path below, as does everything when the window
      // is disabled (/OPT:GCTYPES -- see g_debug_s_window).
      ...
      // the copy. Readers pick the copy up through lnk_obj_get_sect_data. The copy bytes go on
      // dedicated per-worker SECT_DATA_COPIES arenas (free-list warm-page reuse, unlike raw
      // reserve+commit) so lnk_build_pdb can hand the whole set back with arena_release once
      // the last $S reader is done -- GB-class at FN scale (the String8 table itself stays on
      // the worker arena; it must outlive the copies for the zeroing sweep).
    ...
    lnk_obj_apply_relocs_to_buffer(obj, sect_idx, section_header, section_data, task->image_base, task->image_section_table);
  }
...
lnk_build_image(TP_Arena *arena, TP_Context *tp, LNK_Config *config, LNK_SymbolTable *symtab, U64 objs_count, LNK_Obj **objs)
{
...

  {
    ...
    // patch relocs
    {
      ...
      tp_for_parallel_prof(tp, arena, objs_count, lnk_obj_reloc_patcher, &task, "Patch Relocs"); // arena: the per-obj sect_data_copies String8 tables
    }
