...
THREAD_POOL_TASK_FUNC(lnk_walk_relocs_and_mark_ref_sections_task)
{
  ...
  // the debug log totals are identical regardless of cohort width or schedule.
  {
    ...
    for (U64 obj_idx = task_id; obj_idx < objs.count; obj_idx += tp->worker_count) {
...
{
  // scalar fields only here; the name is decoded (memoized) below only for the rare
  // non-Regular fallback that keys the target by name hash.
  COFF_ParsedSymbol          tp   = lnk_parsed_symbol_from_coff_symbol_idx_no_name(c->obj, reloc->isymbol);
  ...
  if (ti == COFF_SymbolValueInterp_Undefined || ti == COFF_SymbolValueInterp_Weak) {
    ...
    if (lnk_resolve_symbol(task->symtab, tref, &resolved)) {
      tref = resolved;
      tp   = lnk_parsed_symbol_from_coff_symbol_idx_no_name(tref.obj, tref.symbol_idx);
  ...
  if (ti == COFF_SymbolValueInterp_Regular && tref.obj != 0) {
    ...
    // tp/tref stay in sync above, so decode the (memoized) name for the current ref
    String8 tname = lnk_symbol_name_from_coff_symbol_idx(tref.obj, tref.symbol_idx);
    U64 nh = 14695981039346656037ull;
    for (U64 i = 0; i < tname.size; i += 1) { nh = lnk_icf_mix(nh, tname.str[i]); }
    target = lnk_icf_mix(nh, tp.value);
  }

  task->rt_iscand[idx] = iscand;
  task->rt_target[idx] = target;
  // candidate targets: identity refines via colors, but the interior OFFSET must be keyed.
  // Switch jump tables reference case blocks through section-local labels ($LN..., value =
  // case offset) with zero inline addend, so two byte-identical functions whose tables map
  // cases to DIFFERENT blocks differ only in these values -- dropping them folds functions
  // with different dispatch.
  LNK_ICFRelocKeyEntry e;
  e.type      = reloc->type;
  e.iscand    = iscand;
  e.apply_off = reloc->apply_off;
  e.target    = iscand ? tp.value : target; // interior offset for candidate targets (see comment above)
...
// symbol, chase Undefined/Weak through the symbol table, return its value.
internal U64
lnk_icf_reloc_resolved_value(LNK_SymbolTable *symtab, LNK_Obj *obj, COFF_Reloc *reloc)
{
  COFF_ParsedSymbol          tp = lnk_parsed_symbol_from_coff_symbol_idx(obj, reloc->isymbol);
  COFF_SymbolValueInterpType ti = coff_interp_from_parsed_symbol(tp);
  if (ti == COFF_SymbolValueInterp_Undefined || ti == COFF_SymbolValueInterp_Weak) {
    LNK_ObjSymbolRef resolved = {0};
    if (lnk_resolve_symbol(symtab, (LNK_ObjSymbolRef){ obj, reloc->isymbol }, &resolved)) {
      tp = lnk_parsed_symbol_from_coff_symbol_idx(resolved.obj, resolved.symbol_idx);
    }
  }
  return tp.value;
}
...
// match pairwise. key0 hashes all three, so same-class candidates normally agree -- this is the
// hash-collision guard, the reloc-side counterpart of the byte compare. The caller class-compares
// target IDENTITY via the rt slice; the offset matters independently (see lnk_icf_key_reloc).
// Walk order mirrors lnk_icf_hash_task (own relocs, then associative children in gather order);
// the caller has already matched per-section reloc counts.
internal B32
...
// first associated .debug$S child of a function section (name-checked so an associated
// .debug$T can never be mistaken for symbol/line data)
internal U32
...
THREAD_POOL_TASK_FUNC(lnk_icf_mark_folded_lines_task)
{
  ...
  for EachIndex(sect_idx, obj->header.section_count_no_null) {
    ...
    for EachNode(assoc_n, U32Node, obj->associated_sections[sect_idx + 1]) {
      ...
      obj->icf_lines_only[assoc_sn - 1] = mark;
    }
  }
        ...
        if (interp == COFF_SymbolValueInterp_Regular) {
          if (symbol.section_number == lnk_obj_get_removed_section_number(obj)) {
            if (~section_flags & LNK_SECTION_FLAG_DEBUG) {
              ...
              lnk_error_obj(LNK_Error_RelocationAgainstRemovedSection, obj, "relocating against symbol that is in a removed section (symbol: %S, reloc-section: %S 0x%llx, reloc-index: 0x%llx)", lnk_symbol_name_from_coff_symbol_idx(obj, reloc->isymbol), sect_name, sect_idx+1, reloc_idx);
            }
            continue;
          }
...
lnk_run_linker(TP_Context *tp, TP_Arena *arena, LNK_Config *config)
{
  ...
  {
    U64 input_bytes = 0;
    for EachIndex(obj_idx, objs_count) { input_bytes += objs[obj_idx]->data.size; }
...
entry_point(CmdLine *cmdline)
{
  ...
  // summary: identity fields, copied IMMEDIATELY after the parse -- @rsp-parsed
  // config strings can point into the response-file scratch, which is reused by
  // the very next pushes on this thread (observed as garbage pool= names when
  // the copy happened after tp_alloc)
