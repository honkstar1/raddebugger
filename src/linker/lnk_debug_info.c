  ...
  // window labels a folded frame with that source's own variable names.
  {
    ...
    if (obj->icf_lines_only != 0) {
      for EachIndex(sect_idx, obj->header.section_count_no_null) {
        ...
        cv_debug_s_tag_prov_sect(&ds, (U32)sect_idx); // nodes merged from a DIFFERENT (child) section: record which
...
lnk_leaf_ref_raw_size(LNK_CodeViewInput *input, LNK_LeafRef leaf_ref)
{
  ...
  if (leaf_ref.obj_idx < input->obj_count && lnk_notype_journal_test(&input->notype_journal[leaf_ref.obj_idx], leaf_ref.leaf_idx)) {
    B32 kind_only = 0;
    lnk_notype_journal_find(&input->notype_journal[leaf_ref.obj_idx], leaf_ref.leaf_idx, &kind_only);
    if (!kind_only) { return sizeof(CV_LeafHeader); }
  }
  return cv_debug_t_get_raw_leaf(debug_t, leaf_ref.leaf_idx).size;
}
...
lnk_hash_cv_leaf(LNK_CodeViewInput *input, Arena *journal_arena, LNK_LeafRef leaf_ref, CV_Leaf leaf, CV_TiOffsets ti_offs, B32 discard_cycles)
{
  ...
  for (U64 ti_idx = 0; ti_idx < ti_count; ti_idx += 1) {
    ...
    if (sub_ti >= debug_t->ti_ranges[sub_ti_n.source].max) {
      ...
      if (leaf_ref.obj_idx < input->obj_count) {
        lnk_notype_journal_push(journal_arena, &input->notype_journal[leaf_ref.obj_idx], leaf_idx, 0, debug_t->count);
      } else {
      ...
      if (leaf_idx == leaf_ref.leaf_idx) { self_discarded = 1; }
  ...
  } else if (leaf_ref.obj_idx < input->obj_count &&
             lnk_notype_journal_test(&input->notype_journal[leaf_ref.obj_idx], leaf_ref.leaf_idx)) {
    CV_LeafHeader leaf_header = { .size = (CV_LeafSize)(leaf.data.size + sizeof(CV_LeafKind)), .kind = leaf.kind };
    lnk_hasher_update_struct(&hasher, &leaf_header);
  } else {
    lnk_hasher_update_struct(&hasher, cv_debug_t_get_leaf_header(debug_t, leaf_ref.leaf_idx));
  }
                      ...
                      CV_TiOffsets         root_ti_offs)
{
  ...
  root_frame->leaf         = lnk_cv_leaf_from_leaf_ref(input, root_leaf_ref.obj_idx, root_leaf_ref.leaf_idx);
  ...
  while (stack) {
    while (stack->ti_next < stack->ti_count) {
      ...
      frame->leaf         = lnk_cv_leaf_from_leaf_ref(input, leaf_ref.obj_idx, leaf_ref.leaf_idx);
...
THREAD_POOL_TASK_FUNC(lnk_hash_debug_t_task)
{
  ...
  for EachIndex(leaf_idx, debug_t->count) {
    ...
    lnk_hash_cv_leaf(task->input, arena, (LNK_LeafRef){ obj_idx, leaf_idx }, leaf, ti_offs, 1);
    temp_end(temp);
  }
  ProfEnd();
}
...
lnk_obj_window_debug_s(Arena *arena, LNK_CodeViewInput *cv, U64 obj_idx, U64 image_base, COFF_SectionHeader **image_section_table, B32 symbols_only)
{
  ...
  U64  sect_count = obj->header.section_count_no_null;
  ...
  for EachElement(k, src->data_list) {
    ...
    for (CV_DebugSProvNode *prov = src->prov_list[k].first; prov != 0; prov = prov->next) {
      ...
      if (is_journal_kind) {
        ...
        COFF_SectionHeader *hdr = &lnk_coff_section_table_from_obj(obj)[prov->sect_idx];
        if (coff_reloc_info_from_section_header(obj->data, hdr).count > 0) { state = 2; }
      }
  ...
  for EachIndex(sect_idx, sect_count) {
    ...
    LNK_ObjSection section = lnk_obj_section_from_sect_idx(obj, sect_idx);
    String8        raw     = str8_substr(obj->data, section.frange); // deliberately NOT lnk_obj_get_sect_data: window fills always start from the immutable raw view
    if (sect_state[sect_idx] == 1) {
      ...
      lnk_obj_apply_relocs_to_buffer(obj, sect_idx, section.header, str8(copy, raw.size), image_base, image_section_table);
  ...
  if (!symbols_only) {
    ...
    for (String8Node *n = src->data_list[idx_strtab].first; n != 0 && prov != 0; n = n->next, prov = prov->next) {
      ...
      if (sect_state[prov->sect_idx] == 2) {
        LNK_ObjSection section = lnk_obj_section_from_sect_idx(obj, prov->sect_idx);
        String8        raw     = str8_substr(obj->data, section.frange);
...
THREAD_POOL_TASK_FUNC(lnk_materialize_unique_leaves_task)
{
  ...
  for EachInRange(i, range) {
    ...
    U64 raw_size;
    if (leaf_ref->obj_idx < task->input->obj_count &&
        lnk_notype_journal_test(&task->input->notype_journal[leaf_ref->obj_idx], leaf_ref->leaf_idx)) {
      B32 kind_only = 0;
      lnk_notype_journal_find(&task->input->notype_journal[leaf_ref->obj_idx], leaf_ref->leaf_idx, &kind_only);
      if (kind_only) {
...
// nodes (no backing section). Valid only while lnk_obj_get_sect_data still returns what the
    ...
    if (task_id == 0) {
      if (task->free_sect_copies) {
        ProfScope("Release Sect Data Copies") {
          for EachIndex(obj_idx, task->cv->obj_count) {
            lnk_obj_drop_sect_data_copies(task->cv->obj_arr[obj_idx]);
          }
...
lnk_build_pdb(TP_Context *tp, TP_Arena *tp_arena, String8 image_data, LNK_Config *config, LNK_SymbolTable *symtab, LNK_CodeViewInput *cv, LNK_MergedTypes cv_types, LNK_PdbWriter writer, LNK_PDB_BuilderFlags builder_flags, struct LNK_Inputer *inputer)
{
  ...
  // patched debug-section copies (obj->sect_data_copies) release at the END of the
